...
describe("deriveDraftStatus", () => {
  it("submitted and error take precedence over everything", () => {
    expect(
      deriveDraftStatus({ hasContent: true, counts: counts({ S0: 3 }), submitted: true, lastSendFailed: false })
    ).toBe("submitted");
    expect(
      deriveDraftStatus({ hasContent: true, counts: counts({ S0: 3 }), submitted: false, lastSendFailed: true })
    ).toBe("error");
  });
  it("empty content is unfinished", () => {
    expect(
      deriveDraftStatus({ hasContent: false, counts: counts(), submitted: false, lastSendFailed: false })
    ).toBe("unfinished");
  });
